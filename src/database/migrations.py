...
def create_database_indexes():
    ...
    with get_starbase_session() as session:
        existing_indexes = {
            row[0] for row in session.execute(text(existing_indexes_query))
        }
        ...
        try:
            ...
            if created_count > 0:
                session.commit()
                logger.info(
                    f"Successfully created {created_count} new database indexes"
                )
            else:
                logger.info(
                    "All database indexes already exist, no new indexes created"
                )
