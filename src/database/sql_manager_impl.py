...
def _get_accession_mode(accessions):
    if accessions:
        if all(str(a).startswith("SSA") for a in accessions):
            return "SSA"
        if all(str(a).startswith("SSB") for a in accessions):
            return "SSB"
    return None


def _normalize_accession_tags(accession_tags=None, accessions=None):
    tags = accession_tags if accession_tags is not None else accessions
...
def _fetch_meta_data_full():
    ...
    # j3k4l5m6n7o8_* for is_deleted filtering, k4l5m6n7o8p9_* for genus/species/class)
    # mirroring the join previously duplicated here as an inline query.
    meta_query = """
...
def fetch_ships(
    ...
    with _session() as session:
        # ships_with_metadata is a real SQL view (see alembic/versions/i2j3k4l5m6n7_*)
        # mirroring the join previously duplicated here as an inline CTE.
        if with_sequence:
            select_cols = """
                sm.ship_id,
                sm.accession_tag,
...
def _fetch_ship_table_full():
    # ship_table_view mirrors this narrower join (see alembic/versions/i2j3k4l5m6n7_*)
    # -- no features/genomes/navis/haplotype/captains, to avoid row fan-out.
...
def fetch_accession_ship(accession_tag):
    ...
    WHERE j.ship_id IS NOT NULL AND sa.ship_accession_tag = :accession_tag AND s.sequence IS NOT NULL
    """
    ...
    LEFT JOIN ship_accessions sa ON sa.ship_id = j.ship_id
    LEFT JOIN accessions a ON a.id = j.accession_id
    WHERE j.ship_id IS NOT NULL AND sa.ship_accession_tag = :accession_tag AND g.source IS NOT NULL
    """
    ...
    with _session() as session:
        try:
            sequence_df = pd.read_sql_query(
                sequence_query,
                session.bind,
                params={"accession_tag": accession_tag},
            )
...
def fetch_captains(
    ...
    # captains_with_metadata is a real SQL view (see alembic/versions/i2j3k4l5m6n7_*)
    # mirroring the join previously duplicated here as an inline CTE.
    where_clauses = ["1=1"]
    if accessions:
