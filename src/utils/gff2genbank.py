...
def gff2genbank(gff_file, fasta_file):
    out_file = tempfile.NamedTemporaryFile(suffix=".gb", delete=False).name

    fasta_input = SeqIO.to_dict(SeqIO.parse(fasta_file, "fasta"))
    gff_iter = GFF.parse(gff_file, fasta_input)
    SeqIO.write(
        _check_gff(_fix_ncbi_id(_extract_regions(gff_iter))), out_file, "genbank"
    )
...
def _check_gff(gff_iterator):
    ...
    for rec in gff_iterator:
        if isinstance(rec.seq, Seq.UndefinedSequenceError):
