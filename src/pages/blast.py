...
layout = html.Div([
    dbc.Container([
        ...
        html.H2("Search protein/nucleotide sequences for Starships and Starship-associated genes."),
        html.Br(),
        dbc.Row([
                        ...
                        html.Table(style={"width": "100%"}, children=[
                            html.Tr([
                                html.Td(style={"width": "50%"}, children=[
                                    dcc.Textarea(id='query_text', placeholder='Paste FASTA sequence here...', rows=5, style={'width': '75%'})                                        
                                ]),
                                html.Td(style={"width": "50%"}, children=[
                                    dcc.Upload(
                                        id='query_file',
                                        children=html.Button('Upload FASTA File')
                                    )                                        
                                ])
                            ])
                        ])
...
def check_input(n_clicks, query_text_input, query_file_contents, query_filename):
    ...
    if ((query_text_input == "" and query_text_input is None) and query_file_contents is None):
        return html.Div([
            html.Div("No input provided:", style={'color': 'red'}),
            html.Div("Please provide either a query sequence in the text box, or upload a fasta file", style={'color': 'red'})
        ])
    elif ((query_text_input != "" and query_text_input is not None) and query_file_contents is not None):
        return html.Div([
            html.Div("Multiple inputs:", style={'color': 'red'}),
            html.Div("Please provide either a query sequence in the text box, or upload a fasta file, not both", style={'color': 'red'})
        ])
        ...
        return html.Div([
            ...
            html.Div("This was not supposed to happen... Try again?", style={'color': 'red'})
        ])
    ...
    query_seq = SeqRecord(Seq(query_list['cleaned_query']), id=query_header, description="")
    
    tmp_fasta = tempfile.NamedTemporaryFile(suffix=".fa").name
    SeqIO.write(query_seq, tmp_fasta, "fasta")

    if n_clicks > 0:
        blast_table = run_blast(seq_type=query_list['query_type'], blast_type="blastn", tmp_fasta=tmp_fasta, input_eval=0.01, threads=2)
        ...
        return dbc.Spinner(html.Span("Processing...")), "Button clicked!", blast_table_ui
...
def parse_fasta_header(query):
    ...
    if header_grep > 1:
        return html.Div([
            html.Div("Multi-fastas not supported at this time", style={'color': 'red'}),
            html.Div("Please provide one sequence at a time.", style={'color': 'red'})
        ])
    elif header_grep == 0:
        query_header = "QUERY"
    elif header_grep == 1:
...
def run_blast(seq_type=None, blast_type=None, tmp_fasta=None, input_eval=None, threads=None):
    
    from Bio.Blast.Applications import NcbiblastnCommandline, NcbiblastpCommandline, NcbitblastnCommandline
    ...
    blast_cline = blast_program(query=tmp_fasta, db=blastdb, evalue=input_eval, out=blast_tmp, outfmt=6, num_threads=threads)
    stdout, stderr = blast_cline()

    # Read the BLAST results into a DataFrame
    blast_results = pd.read_csv(blast_tmp, sep='\t', names=["query_id", "hit_IDs", "pident", "aln_length", "mismatches", "gaps", "query_start", "query_end", "subject_start", "subject_end", "evalue", "bitscore", "query_seq", "subject_seq"])
    ...
    return html.Div([
            ...
            dash_table.DataTable(
                id='blast_table',
                columns=[
                    {"name": i, "id": i, "deletable": False, "selectable": True} for i in blast_results.columns
                ],
