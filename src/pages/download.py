...
@callback(
    [Output("dl-table-container", "children"), Output("table-stats", "children")],
    [Input("curated-input", "checked"), Input("dereplicated-input", "checked")],
)
def update_dl_table(curated, dereplicate):
    logger.debug(
        f"update_dl_table called with curated={curated}, dereplicate={dereplicate}"
    )
    ...
    try:
        ...
        records = [
            {k: str(v) if pd.notnull(v) else "" for k, v in record.items()}
