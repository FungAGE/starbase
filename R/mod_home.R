...
#'
#' @import markdown
#' @importFrom shiny NS tags tagAppendAttributes tagList
      ...
      # tagList(
        tags$table(style = "width: 85%",
          ...
          tags$tr(tags$td(style = "width: 100%",
                          align = "middle",
                          column(width=4,
                            box(width=NULL,title="What can I do with starbase?",status="primary",solidHeader = TRUE,
                              working_ul)),
                          column(width=4,
                            box(width=NULL,title="This version of starbase is under active development",status="warning",solidHeader = TRUE,
                              tags$table(style = "width: 85%",
    ...
    fluidRow(
      column(width=8,
        box(width=NULL,title = "Data Availability",status="primary",solidHeader = TRUE,
          p("We have been maintaining starbase data on our GitHub repo (currently private). We are currently in the process of migrating to a new back-end, which will provide more options for data export. In the mean time, you can retrieve all Starship sequences, annotations, and more, in a single .zip file (size ~30Mb)"),
