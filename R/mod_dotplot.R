...
mod_dotplot_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    ref<-"/home/data/Starships/ships/fna/starfish/aspcri2_s00912.fna"
    query<-"/home/data/Starships/ships/fna/starfish/morimp1_s08010.fna"
    # -b int
      ...
      if (is.null(infile)) {
        ...
        if (opt$on_target & length(levels(alignments$refID)) > 1) {
          ...
          scaffoldIDmean <- sapply(unique(as.character(alignmentsOnTarget$queryID)), function(x) {
            weighted.mean(
              x = alignmentsOnTarget$percentID[which(as.character(alignmentsOnTarget$queryID) == x)],
          ...
          scaffoldIDmean <- sapply(unique(as.character(alignments$queryID)), function(x) {
            weighted.mean(
              x = alignments$percentID[which(as.character(alignments$queryID) == x)],
      ...
      if (is.null(input$file1)) {
        ...
        if (opt$similarity) {
          gp <- ggplot(mydata) +
            ...
            scale_x_continuous(
              breaks = cumsum(as.numeric(chromMax)),
            ...
            labs(
              color = "Mean Percent Identity (per query)",
              title = paste0(
                paste0("Post-filtering number of alignments: ", nrow(mydata), "\t\t\t\t"),
          ...
          gp <- ggplot(mydata) +
            geom_point(
              mapping = aes(x = refStart2, y = queryStart2),
              size = 0.009
            ) +
            geom_point(
              mapping = aes(x = refEnd2, y = queryEnd2),
              size = 0.009
            ) +
            geom_segment(aes(
              x = refStart2,
