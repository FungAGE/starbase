...
def parse_paperblast_results(html_content):
    ...
    results = []
    for p_element in soup.find_all('p', style=lambda x: x and 'margin-top: 1em; margin-bottom: 0em;' in x):
        species = p_element.find_next('i').text.strip()
        gene_id = p_element.find_next('a').text.strip()
        evidence = p_element.find_next('span', style='font-family: sans-serif; font-size: smaller;').text.strip()
        ncbi_prot_link = p_element.find('a')['href']

        # Extracting additional information from the nested <ul> elements
        ul_element = p_element.find_next('ul', style='margin-top: 0em; margin-bottom: 0em;')
        li_element = ul_element.find_next('li', recursive=False)
        paper_title = li_element.find('a').text.strip()
        authors = li_element.find_next('small', recursive=False).find_next('a', recursive=False).get('title')
        nested_ul = li_element.find_next('ul',recursive=False)
        paper_excerpt = nested_ul.text.strip()  
        results.append({'species': species, 'gene_id': gene_id, 'evidence': evidence, 'ncbi_prot_link': ncbi_prot_link, 'authors': authors, 'paper_title': paper_title, 'paper_excerpt': paper_excerpt})

    return results

def save_results_to_tsv(results, output_file):
    with open(output_file, 'w', newline='', encoding='utf-8') as tsvfile:
        fieldnames = ['species', 'gene_id', 'evidence', 'ncbi_prot_link', 'paper_title', 'authors', 'paper_excerpt']
        writer = csv.DictWriter(tsvfile, fieldnames=fieldnames, delimiter='\t')
        writer.writeheader()
        writer.writerows(results)

def main():
    parser = argparse.ArgumentParser(description="Submit a search to PaperBLAST and save results in a TSV file.")
