...
limiter = Limiter(get_client_ip, app=server, storage_uri="memory://", default_limits=[])
...
def telemetry_health():
    ...
    try:
        ...
        return {"status": "healthy", "record_count": result}, 200
    except Exception as e:
...
def refresh_cache():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "Cache cleared and old files cleaned up"}
        ), 200
    except Exception as e:
...
def get_accession_details(accession_id):
    try:
        ...
        return jsonify(
            {"content": component_to_dict(content), "title": component_to_dict(title)}
        )
    except Exception as e:
