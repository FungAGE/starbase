...
function plotMacroSynteny(macroSyntenyData) {
  ...
  var colors = d3.quantize(
    d3.interpolateRgb.gamma(2.2)(queryChrColor, subjectChrColor),
  ...
  if (macroSyntenyData.plotMode === "circular") {
    ...
    // remove old svgs
    d3.select("#macroSyntenyBlock").select("svg").remove() // remove svg first
    d3.select("#geneDensityBlock").select("svg").remove() // remove geneDensity plot
    d3.select("#microSyntenyBlock").select("svg").remove() // remove microSynteny also
    //d3.select("#macroSyntenyBlock")
    //    .append(() => svg.node());
    // create svg
    const svg = d3
      .select("#macroSyntenyBlock")
      .append("svg")
      .attr("viewBox", [-width / 2, -height / 2, width, height])
    ...
    queryChrInfo = calc_circular_angle(
      queryChrInfo,
      Math.PI,
      2 * Math.PI,
      padAngle
    ...
    var queryTickStep = d3.tickStep(
      0,
      d3.sum(queryChrInfo.map((e) => e.value)),
      40
    )
    ...
    var subjectTickStep = d3.tickStep(
      0,
      d3.sum(subjectChrInfo.map((e) => e.value)),
      40
    )
    ...
    ribbonData.forEach((d) => {
      ...
      let targetChr = subjectChrInfo.filter(
        (e) => e.data.chr === d.subjectChr
      )[0]
      ...
      const x = d3
        ...
        .range([
          sourceChr.startAngle + sourceChr.padAngle / 2,
      ...
      const y = d3
        ...
        .range([
          targetChr.startAngle + targetChr.padAngle / 2,
          targetChr.endAngle - targetChr.padAngle / 2,
        ])

      const sourceOutAngle = {
        startAngle: x(d.queryStart),
        endAngle: x(d.queryEnd),
      }

      const targetOutAngle = {
        startAngle: y(d.subjectStart),
      ...
      if (d.orientation === "+") {
        d.ribbonAngle = {
          source: {
            startAngle: sourceOutAngle.startAngle,
            endAngle: sourceOutAngle.endAngle,
          },
          target: {
            startAngle: targetOutAngle.startAngle,
            endAngle: targetOutAngle.endAngle,
          },
        }
      } else {
        d.ribbonAngle = {
          ...
          target: {
            startAngle: targetOutAngle.endAngle,
            endAngle: targetOutAngle.startAngle,
          },
        }
      }
    })
    ...
    const queryGroupTick = queryGroup
      .append("g")
      .selectAll("g")
      .data((d) => d.tickData)
      .join("g")
    ...
    queryGroupTick.append("line").attr("stroke", "currentColor").attr("x2", 4)
    ...
    subjectGroup
      ...
      .attr(
        "fill",
        (d) => colors[queryChrInfo.length + subjectChrInfo.length - d.index - 1]
      )
      .attr("d", arc)
    ...
    subjectGroup
      .append("text")
      .attr("dy", function (d, i) {
        return d.startAngle > (90 * Math.PI) / 180 &&
          d.startAngle < (270 * Math.PI) / 180
          ? 35
          : -28
      })
    ...
    const subjectGroupTick = subjectGroup
      .append("g")
      .selectAll("g")
      .data((d) => d.tickData)
      .join("g")
    ...
    subjectGroupTick.append("line").attr("stroke", "currentColor").attr("x2", 4)
    ...
    // plot query chrs
    const queryGroup = svg.append("g").attr("class", "macroQueryGroup")

    const subjectGroup = svg.append("g").attr("class", "macroSubjectGroup")
    ...
    queryGroup
      ...
      .attr(
        "y",
        topPadding + d3.select("#queryMainLabel").node().getBBox().height
      )
      .attr("font-weight", "bold")
    ...
    queryGroup
      ...
      .attr("x", (d) =>
        d3.mean([queryScale(d.accumulate_end), queryScale(d.accumulate_start)])
      )
      .attr("y", function () {
        return (
          Number(d3.select("#queryMainLabel").attr("y")) +
          Number(5) +
          Number(d3.select(this).node().getBBox().height)
        )
      })
      .attr("font-weight", "bold")
      .attr("font-size", "1rem")
    ...
    queryGroup
      ...
      .attr(
        "y",
        topPadding +
          d3.select("#queryMainLabel").node().getBBox().height +
          5 +
          d3
            .selectAll(".macroQueryGroup text")
            .filter(":not(#queryMainLabel)")
            .node()
            .getBBox().height +
          5
    ...
    subjectGroup
      ...
      .attr("x", (d) =>
        d3.mean([
          subjectScale(d.accumulate_end),
          subjectScale(d.accumulate_start),
        ])
      )
      .attr(
        "y",
        d3.select("#subjectMainLabel").attr("y") -
          d3.select("#subjectMainLabel").node().getBBox().height -
          5
      )
      .attr("font-weight", "bold")
      .attr("font-size", "1rem")
    ...
    subjectGroup
      ...
      .attr(
        ...
        height -
          ...
          5 -
          d3
            .selectAll(".macroSubjectGroup text")
            .filter(":not(#subjectMainLabel)")
            .node()
            .getBBox().height -
          5 -
    ...
    ribbonData.forEach((d) => {
      ...
      let queryAccumulateStart =
        queryChr.accumulate_start + d.queryStart - queryChr.start + 1
      let queryAccumulateEnd =
        queryChr.accumulate_start + d.queryEnd - queryChr.start + 1
      let subjectAccumulateStart =
        subjectChr.accumulate_start + d.subjectStart - subjectChr.start + 1
      let subjectAccumulateEnd =
        subjectChr.accumulate_start + d.subjectEnd - subjectChr.start + 1
      if (d.orientation === "+") {
        d.ribbonPosition = {
          source: {
  ...
  // Activate tooltips
  tippy(".macroQueryArc path", {
    trigger: "mouseenter",
    followCursor: "initial",
    delay: [tooltipDelay, null],
  })
  tippy(".macroSubjectArc path", {
    trigger: "mouseenter",
    followCursor: "initial",
    delay: [tooltipDelay, null],
  })
  tippy(".macroQueryGroup rect", {
    trigger: "mouseenter",
    followCursor: "initial",
    delay: [tooltipDelay, null],
  })
  tippy(".macroSubjectGroup rect", {
    trigger: "mouseenter",
    followCursor: "initial",
    delay: [tooltipDelay, null],
  })
  tippy(".macroRibbons path", {
    trigger: "mouseenter",
    followCursor: "initial",
    allowHTML: true,
    delay: [tooltipDelay, null],
...
function calc_circular_angle(inputChrInfo, startAngle, endAngle, padAngle) {
  const arcs = d3
    ...
    .endAngle(endAngle)
    .padAngle(padAngle)
    .value((d) => d.chrLength)(inputChrInfo)
  return arcs
}
...
function ribbonCustom(d, radius, padAngle) {
  ...
  // add pading
  if (ap > Math.epsilon) {
    if (Math.abs(sa1 - sa0) > ap * 2 + Math.epsilon)
      sa1 > sa0 ? ((sa0 += ap), (sa1 -= ap)) : ((sa0 -= ap), (sa1 += ap))
    else sa0 = sa1 = (sa0 + sa1) / 2
    if (Math.abs(ta1 - ta0) > ap * 2 + Math.epsilon)
...
function plotSelectedMicroSynteny(microSyntenyData) {
    ...
    micro_anchors.forEach((e) => {
      if (
        ...
        d.pathData.push({
          ...
          sourceElementID:
            "queryGene_" +
            micro_queryBed.findIndex((element) => element.gene === e.q_Gene),
          targetElementID:
            "subjectGene_" +
            micro_subjectBed.findIndex((element) => element.gene === e.s_Gene),
  ...
  if (queryGeneNum < 100) {
    ...
    var selectedRegionData = regionData.slice(
      regionData.length - 4,
      regionData.length
    )
  }
  ...
  const heatmapSVG = d3
    ...
    .attr("viewBox", [
      ...
      0,
      width,
      heatmapMargin.top + heatmapHeight + heatmapMargin.bottom,
    ])
  // Add heatmap rects
  heatmapSVG
    .append("g")
  ...
  brush = d3
    .brushX()
    .extent([
      [heatmapMargin.left, heatmapMargin.top - 5],
      [width - heatmapMargin.right, heatmapHeight + heatmapMargin.top + 5],
    ])
    .on("end", brushended)

  const heatmapAxisHorizental = heatmapSVG.append("g").call(xAxis, hScale)

  // define defaultSelection for brush
  let defaultSelection = [
    hScale(regionData[regionData.length - 5].start),
    hScale.range()[1],
  ]
  ...
  function brushended(event) {
    ...
    let x0 = null,
      x1 = null
    const selectedWidth =
      hScale.invert(selection[1]) - hScale.invert(selection[0])
    ...
    regionData.forEach((e, i) => {
      if (
        hScale.invert(selection[0]) >= e.start &&
        hScale.invert(selection[0]) <= e.end
      ) {
      ...
      if (
        hScale.invert(selection[0]) < e.start &&
        hScale.invert(selection[1]) > e.end
      ) {
        selectedDataIdx.push(i)
      }
      if (
        hScale.invert(selection[1]) >= e.start &&
        hScale.invert(selection[1]) <= e.end
      ) {
    ...
    d3.select(this)
      .transition()
      .call(brush.move, x1 > x0 ? [x0, x1].map(hScale) : null)
    ...
    // remove rects first
    d3.select("#microSyntenyBlock").select("svg").remove()
    selectedRegionData = selectedDataIdx.map((d) => regionData[d])

    const microSVG = generate_microSytenySVG(
      selectedRegionData,
      micro_subjectBed,
      querySpecies,
      subjectSpecies,
    ...
    d3.select("#microSyntenyBlock").select("svg").remove()
    d3.select("#microSyntenyBlock").append(() => microSVG.node())
    // Activate tippy tooltips
    tippy(".microQueryGroup rect", {
      trigger: "mouseenter",
      followCursor: "initial",
      delay: [tooltipDelay, null],
    })
    tippy(".microSubjectGroup rect", {
      trigger: "mouseenter",
      followCursor: "initial",
      delay: [tooltipDelay, null],
    })
    tippy(".microRibbons path", {
      trigger: "mouseenter",
      allowHTML: true,
      followCursor: "initial",
      delay: [tooltipDelay, null],
  ...
  d3.select("#geneDensityBlock").select("svg").remove() // remove svg first
  d3.select("#microSyntenyBlock").select("svg").remove()
  ...
  if (queryGeneNum > 100) {
    d3.select("#geneDensityBlock").append(() => heatmapSVG.node())
  }
  d3.select("#microSyntenyBlock").append(() => microSVG.node())
  downloadSVG("microSynteny_download", "microSyntenyBlock")
  // Add tooltips
...
function plot_microSyntenyGenes(
  svg,
  myData,
  xScale,
  yaxis,
  ...
  const g = svg.append("g").attr("class", groupClass)
...
function createPath(svg, pathData, ribbonColor, pathClass = "ribbons") {
  ...
  // draw band path
  const pathGroup = svg.append("g").attr("class", pathClass)
  ...
  pathGroup
    ...
    // })
    .on("mouseout", function () {
      d3.select(this).transition().duration(50).style("fill-opacity", 0.6)
      let sourceRectID = this.__data__.source.id
      let targetRectID = this.__data__.target.id
      d3.select("#" + sourceRectID)
        .transition()
...
function createLinkPolygonPath(d) {
  ...
  return (
    ...
    x +
    "," +
    y2 + // 1st curve point 1
    " " +
    x1 +
    "," +
    y3 + // 1st curve point 2
    " " +
    x1 +
    "," +
    y1 + // 1st curve end point
    "L" +
    p0 +
    "," +
    y1 + // bottom line
    ...
    p0 +
    "," +
    q2 + // 2nd curve point 1
    " " +
    p1 +
    "," +
    q3 + // 2nd curve point 2
    " " +
    p1 +
    "," +
    y
  ) // end point and move back to start
}
...
function generate_microSytenySVG(
  inputRegionData,
  inputSubjectBedData,
  querySpecies,
  subjectSpecies,
  ...
  const svg = d3.create("svg").attr("viewBox", [0, 0, width, plotHeight])
  ...
  let subjectRegionStart = d3.min(
    inputRegionData
      .map((e) => e.anchors.map((d) => d.s_GeneStart).flat())
      .flat()
  )
  let subjectRegionEnd = d3.max(
    inputRegionData.map((e) => e.anchors.map((d) => d.s_GeneEnd).flat()).flat()
  )
  ...
  queryScale = d3
    .scaleLinear()
    .domain([
      queryRegionStart - queryRegionLength * 0.025,
      queryRegionEnd + queryRegionLength * 0.025,
    ])
    .range([0, width])
  svg.append(() =>
    plot_microSyntenyGenes(
      svg,
      queryData,
      queryScale,
      queryY,
  ...
  subjectScale = d3
    .scaleLinear()
    .domain([
      subjectRegionStart - subjectRegionLength * 0.025,
      subjectRegionEnd + subjectRegionLength * 0.025,
    ])
    .range([0, width])
...
function downloadSVG(downloadButtonID, svgDivID) {
  ...
  d3.select("#" + downloadButtonID).on("click", function (e) {
    const chart = d3
      .select("#" + svgDivID)
      .select("svg")
      .node()
    const url = URL.createObjectURL(serialize(chart))
    d3.select(this).attr("download", "output.svg").attr("href", url)
  })
}
...
function downloadSVGwithForeign(downloadButtonID, svgDivID) {
  ...
  d3.select("#" + downloadButtonID).on("click", function (e) {
    ...
    const url =
      ...
      encodeURIComponent(serializeString(chart))
...
Shiny.addCustomMessageHandler(
  ...
  function (selectedQueryGene) {
    ...
    if (regionData.length <= 5) {
      ...
      selectedRegionDataIdx >= 3 &&
      selectedRegionDataIdx <= regionData.length - 3
    ) {
    ...
    d3.select("#geneDensityBlock")
      ...
      .call(
        ...
        [
          ...
          regionData[selectedIdxAll[selectedIdxAll.length - 1]].end,
        ].map(hScale)
      )
    ...
    d3.select(
      "#" +
        "queryGene_" +
        micro_queryBed.findIndex(
          (element) => element.gene === selectedQueryGene
        )
    ).style("fill", "#FF5733")
  }
)
...
function plotDotView(dotViewData) {
  ...
  anchorSeed.forEach((e) => {
    ...
    let queryScale = d3
      .scaleLinear()
      .domain([queryChr.start, queryChr.end])
      .range([queryChr.accumulate_start, queryChr.accumulate_end])
    let subjectScale = d3
      .scaleLinear()
      .domain([subjectChr.start, subjectChr.end])
      .range([subjectChr.accumulate_start, subjectChr.accumulate_end])
  ...
  // remove old svgs
  d3.select("#dotView").select("svg").remove() // remove svg first
  ...
  // init canvas
  const canvas = d3.create("canvas").attr("width", width).attr("height", height) // same size as svg, so the scales could be shared
  ...
  anchorSeed.forEach((e) => {
    drawPoint(xScale(e.accumulate_x), yScale(e.accumulate_y), "black")
  })
  ...
  svg
    ...
    .attr(
      ...
      (d) =>
        `translate(-15 ${yScale(
          d3.mean([d.accumulate_start, d.accumulate_end])
        )})`
    )
  ...
  function zoom() {
    ...
    svg
      ...
      .attr("x", (d) => {
        if (
          ...
          d.accumulate_start < xScale.domain()[1] &&
          d.accumulate_start > xScale.domain()[0] &&
          d.accumulate_end > xScale.domain()[1]
        ) {
          // shift left
    ...
    svg
      ...
      .attr("transform", (d) => {
        ...
        if (
          ...
          d.accumulate_start < yScale.domain()[1] &&
          d.accumulate_start > yScale.domain()[0] &&
          d.accumulate_end > yScale.domain()[1]
        ) {
          // shift up
    ...
    anchorSeed.forEach((e) => {
      if (
        e.accumulate_x >= xScale.domain()[0] &&
